...
public final class Compression {
  ...
  private static final Map<String,CompressionAlgorithm> CONFIGURED_ALGORITHMS =
      StreamSupport.stream(FOUND_ALGOS.spliterator(), false).distinct()
  ...
  public static List<String> getSupportedAlgorithms() {
    return CONFIGURED_ALGORITHMS.values().stream().filter(CompressionAlgorithm::isSupported)
        .map(CompressionAlgorithm::getName).collect(Collectors.toList());
  }
